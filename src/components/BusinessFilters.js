...
const BusinessFilters = () => {
  ...
  // Create a copy of businesses and then sort them alphabetically
  ...
  return (
    <div style={{ textAlign: 'left', height: '100%', overflowY: 'auto', overflowX: 'hidden', scrollbarWidth: 'none', scrollbarColor: 'transparent transparent' }}>
      <h3 style={{ backgroundColor: 'white', padding: '0.5rem 0', margin: 0, position: 'sticky', top: 0, zIndex: 1 }}>Businesses</h3>
      <div style={{ marginTop: '0.5rem', paddingBottom: '1rem' }}>
        {sortedBusinesses.map((business, index) => (
          <div key={index} style={{ display: 'flex', alignItems: 'center' }}>
            <input type="checkbox" id={`business-${index}`} value={business} />
